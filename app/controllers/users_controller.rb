class UsersController < ApplicationController
  ...
  def format_data(users)
    ...
    users.map.with_index do |user, index|
      ...
      current_user_data.push("<div style='text-align: center'><a class='action_icon edit_icon' data-toggle='tooltip' title='Edit user information' data-user_id='#{user.id}' href='javascript:;'><i class='fa fa-pencil icon' style='color: #{pri ? "#fc9803" : "rgb(77, 79, 78)"}'></i></a>
          ...
          <a #{"class='action_icon status_icon'" if pri} data-toggle='tooltip' title='Disable/Enable User' data-user_id='#{user.id}' data-user_account='#{user.account}' href='#'>
